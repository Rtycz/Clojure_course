...
(meditations
  ...
  (= "Rich Hickey aka The Clojurer aka Go Time aka Lambda Guru"
     (let [[first-name last-name & aliases]
       ...
       (str first-name " " last-name " aka " (peek aliases) " aka " (nth aliases 1) " aka "(nth aliases 2))))
  ...
  (= "Test Testerson, 123 Test Lane, Testerville, TX"
     ((fn [[first-name last-name] {:keys [street-address city state]}]
        (apply str (interpose ", " (list (str first-name " " last-name) street-address city state)))) ["Test" "Testerson"] test-address)))
